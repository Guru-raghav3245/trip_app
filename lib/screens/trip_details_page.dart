...
class _TripDetailsPageState extends ConsumerState<TripDetailsPage> {
  ...
  void initState() {
    super.initState();
    tripDates = [];
    for (var date = widget.trip['startDate'];
        date.isBefore(widget.trip['endDate']) ||
            date.isAtSameMomentAs(widget.trip['endDate']);
...
Future<void> _selectFromGallery(DateTime date, String tripId) async {
        ...
        children: [
          ...
          Expanded(
            child: ListView(
              ...
              children: tripDates
                  .map(
                      (date) => _buildDateTile(date, expenses, notes, currency, tripId))
                  .toList(),
            ),
          ),
        ],
 ...
 Widget _buildExpensesSection(
    ...
    String currency,
    String tripId) {
  final imagePathsProvider = ref.watch(imagesProvider(tripId)); // Add image paths provider
          ...
          children: [
            ...
            TextField(
              ...
              onSubmitted: (value) {
                ...
                ref.read(notesProvider(tripId).notifier).updateNote(date, value);
              },
            ),
            const SizedBox(height: 8),
